...
export function buildWriteTools(vault: VaultAdapter, diffEngine: DiffEngine) {
	const proposeEdit = tool({
		...
		description: [
			...
			'Use this for targeted changes; do not rewrite an entire long note unless necessary.'
		].join(' '),
		parameters: z.object({
			path: z.string().describe('The vault-relative path to the note.'),
			find: z.string().min(1).describe('Exact substring of the current note content to replace.'),
			replace: z.string().describe('The text to substitute in place of `find`.'),
			...
			try {
				...
				if (idx < 0) {
					return JSON.stringify({
						error: `Could not find the \`find\` text in ${path}. Use read_note to inspect the current content and try again with an exact match.`
					});
				}
				...
				const proposed = content.slice(0, idx) + replace + content.slice(idx + find.length);
				if (proposed === content) {
